...
async function getSubscriptionCourses(token: string): Promise<CourseWithContent[]> {
  try {
    const response = await fetch('https://carvajalfit.com/courses/subscription', {
      method: 'GET',
      headers: {
...
```bash
curl -X GET "https://carvajalfit.com/courses/subscription" \
...
**Request:**
- Method: `GET`
- URL: `https://carvajalfit.com/courses/subscription`
- Headers:
